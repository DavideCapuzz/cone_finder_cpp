...
void ConeFinder::continuosCallback()
{
  ...
  {
    goal_pose_ = core_.update(in_image_, camera_info_, p_cam_, oc_, odom_, BB_array);
  }
...
void ConeFinder::publishMarker(std::vector<cv::Point> &p_vector)
{
  ...
  for (cv::Point p : p_vector)
  {
    ...
    marker.pose.position.x = p.x * oc_.map_res_ + oc_.map_x0_;
    marker.pose.position.y = p.y * oc_.map_res_ + oc_.map_y0_;
...
void ConeFinder::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg_in)
{
  ...
  try
  {
    ...
    p_cam_ = tools_.position_2_map(bot_camera_tf.position, oc_.map_res_, oc_.map_x0_, oc_.map_y0_);
