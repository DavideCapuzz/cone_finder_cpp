...
void SearchTarget::costmap_cb(const nav_msgs::msg::OccupancyGrid::SharedPtr msg_in)
{
  ...
  if (true) // TODO: check if all the information are updated
  {
    ...
    const int h = static_cast<int>(occupancyGrid.info.height);
    const int w = occupancyGrid.info.width;
    ...
    for (int i{0}; i < w * h; ++i)
    {
      ...
      {
        cost_map[static_cast<int>(i % w + 0.5)][static_cast<int>(i / h + 0.5)] = 100;
      }
    }
    ...
    if (p_100.size() > 0) // check if the cost map has some obstacles
    {
      ...
      cv::Point closest_point;
      tools_.find_nearest_point_2_segment2(closest_point, distance, p_100, bot_pos, p_lateral);
      if (dev_mode_ > 0)
      {
      ...
      if (tools_.distance_points(bot_pos, closest_point) > 0)
      {
        if (dev_mode_ > 0)
        {
          c_mat_image.at<cv::Vec3b>(closest_point) = cv::Vec3b(250, 0, 0);
        }
        bool result{false};
        ...
        do
        {
          ...
          for (int i = -2; i < 2; i++)
          {
            for (int j = -2; j < 2; j++)
            {
              c_m[2 + i][2 + j] = cost_map[static_cast<int>(next_i) + i][static_cast<int>(next_j) + j];
              c_mat_image.at<cv::Vec3b>(
          ...
          if (dev_mode_ > 0)
          {
            RCLCPP_INFO(this->get_logger(), "r %d %d %d", result, next_i, next_j);
          }

          if (dev_mode_ > 0);
          {
            c_mat_image.at<cv::Vec3b>(cv::Point(next_i, next_j)) = cv::Vec3b(250, 0, 0);
          }
        } while (result);
        RCLCPP_INFO(this->get_logger(), "rot %f %d %d", rotation_angle_, next_i, next_j);
        if (line.size()>5)
        {
