...
)
{
  ...
  if (oc_.p_100_.size() > 0) // check if the cost map has some obstacles
  {
    ...
    if (tools_.distance_points(bot_pos, closest_point) > 0)
    {
      if (dev_mode > 0)
      {
        ...
        cv::circle (oc_.im_occgrid_, closest_point, 2, cv::Vec3b(0, 0, 0), 1, cv::LINE_8); // yellow
      }
      bool result{false};
      double next_i = closest_point.x;
      double next_j = closest_point.y;
      //core_.flip();     
      kernel.set_angle(rotation_angle + angle_offset);
      double r{};
      Matrix<5> c_m{};
      int iterations{0};
      do
  ...
  if (dev_mode > 0)
  {
    ...
    if (dev_mode == 1) { cv::waitKey(3); }
    else if (dev_mode > 1) { cv::waitKey(0); }
  }  
  return p_target;
}